<template>
  <div class="fixed inset-0 bg-black flex flex-col md:flex-row z-50">
    ...
    <div class="flex-1 flex flex-col relative group h-full">
      ...
      <button
        @click="goBack"
        class="absolute top-4 right-4 z-20 bg-background/60 hover:bg-background/90 text-foreground p-2 rounded-full backdrop-blur-sm transition-all opacity-100 md:opacity-0 md:group-hover:opacity-100 duration-300 focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
      >
      ...
      <div class="flex-1 bg-black relative w-full h-full">
        ...
        <div v-if="hasContentError" class="absolute inset-0 flex flex-col items-center justify-center text-foreground bg-black/90 z-20">
          ...
          <h2 class="text-2xl font-bold font-poppins mb-2 text-foreground">Próximamente</h2>
          <p class="text-muted-foreground max-w-sm text-center">Este contenido no está disponible por el momento. Mientras tanto, puedes explorar otras recomendaciones.</p>
        </div>
        ...
        />
        <div v-show="!embedUrl && !hasContentError" class="flex items-center justify-center h-full text-foreground">
    ...
    <div class="w-full md:w-96 bg-card border-l border-border hidden md:flex flex-col md:h-full">

      <!-- SERIES SIDEBAR -->
      <template v-if="(type === 'tv' || type === 'anime') && !hasContentError">
        <!-- Header: Season Selector -->
        <div class="p-4 border-b border-border bg-card z-10">
          <BaseSelect
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1">
          ...
          <template v-else>
            <button
              ...
              :class="isCurrentEpisode(ep.episode_number) ? 'bg-muted/60 border-l-2 border-primary' : 'hover:bg-accent border-l-2 border-transparent'"
            >
              ...
              <!-- Thumbnail -->
              <div class="relative flex-none w-28 aspect-video bg-card rounded overflow-hidden">
                <img 
                  v-if="ep.still_path" 
                ...
                <div class="absolute inset-0 flex items-center justify-center">
                   ...
                   <svg 
                      v-else
                      class="w-6 h-6 text-foreground opacity-0 group-hover/item:opacity-80 transition-opacity drop-shadow-md"
                      fill="currentColor" viewBox="0 0 24 24"
                   >
                     <path d="M8 5v14l11-7z"/>
                   </svg>
              ...
              <div class="flex-1 min-w-0 flex flex-col justify-center">
                 <h3 
                  ...
                  :class="isCurrentEpisode(ep.episode_number) ? 'text-primary' : 'text-muted-foreground group-hover/item:text-foreground'"
                 >
                   {{ ep.episode_number }}. {{ ep.name }}
                 </h3>
                <div class="flex items-center text-xs text-muted-foreground space-x-2">
                   <span>{{ ep.runtime ? `${ep.runtime}m` : 'N/A' }}</span>
                   <div v-if="ep.vote_average" class="flex items-center text-muted-foreground">
      ...
      <template v-else>
        <div class="p-4 border-b border-border">
          <h2 class="text-foreground font-bold font-poppins">A continuación</h2>
        </div>
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
          ...
          <div v-else-if="queue.length > 0">
             <div 
               v-for="item in queue" 
               :key="item.id"
               class="flex space-x-3 cursor-pointer hover:bg-accent p-2 rounded-lg transition-colors group"
               @click="playItem(item)"
             >
               <div class="w-24 aspect-video bg-card rounded overflow-hidden flex-shrink-0 relative">
                 <img :src="getBackdropUrl(item.backdrop_path, 'w300')" class="w-full h-full object-cover">
                 <div class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                   <svg class="w-8 h-8 text-foreground" fill="currentColor" viewBox="0 0 20 20"><path d="M6.3 2.841A1.5 1.5 0 004 4.11V15.89a1.5 1.5 0 002.3 1.269l9.344-5.89a1.5 1.5 0 000-2.538L6.3 2.84z" /></svg>
                 </div>
               </div>
               <div class="flex-1 min-w-0">
                 <h4 class="text-foreground text-sm font-medium line-clamp-2 group-hover:text-primary transition-colors">{{ item.title || item.name }}</h4>
                 <p class="text-muted-foreground text-xs mt-1">{{ item.vote_average.toFixed(1) }} rating</p>
               </div>
             </div>
          </div>
          <div v-else class="text-muted-foreground text-sm text-center">
...
function goBack() {
  if (type.value === 'movie') {
    ...
    // Con anime desactivado, cualquier 'anime' cae a serie-detail (ruta que sí existe).
